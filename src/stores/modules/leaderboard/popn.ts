import { defineStore } from "pinia";
import { PnmLeaderboardEntry } from '../../models/leaderboard/entries';

export const usePnmLeaderboardStore = defineStore('pnmLeaderboard', {
  state: () => ({
    leaderboard: [] as PnmLeaderboardEntry[],
    updatedOn: new Date(Date.now())
  }),
  getters: {

  },
  actions: {
    async getLeaderboard() {
      // TODO Fetch from API (Awaiting back-end completion)
      if (!this.leaderboard.length) {
        this.leaderboard = [
          new PnmLeaderboardEntry({
            id: "user1",
            placement: 1,
            name: "username1",
            points: 75,
            flower_id: "flowerUser1",
            username: "flowerUsername1",
            arcade: "Arcade NL",
            nationality: "NL"
          }),
          new PnmLeaderboardEntry({
            id: "user2",
            placement: 2,
            name: "username2",
            points: 75,
            flower_id: "flowerUser2",
            username: "flowerUsername2",
            arcade: "Arcade NL",
            nationality: "NL"
          }),
          new PnmLeaderboardEntry({
            id: "user3",
            placement: 3,
            name: "username3",
            points: 75,
            flower_id: "flowerUser3",
            username: "flowerUsername3",
            arcade: "Arcade NL",
            nationality: "NL"
          }),
        ];
      }
    }
  }
});